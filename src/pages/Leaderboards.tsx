            ...
            <Card>
              ...
              <CardContent>
                  ...
                  {engagementLeaderboard.map((entry) => (
                    <div
                      key={entry.rank}
                      className={`flex flex-col sm:flex-row sm:items-center p-3 sm:p-4 mb-3 rounded-xl shadow-sm bg-white border overflow-x-hidden transition-all hover:shadow-lg ${entry.rank <= 3 ? 'border-yellow-300' : 'border-gray-200'}`}
                    >
                  ...
                  {creativityLeaderboard.map((entry) => (
                    <div
                      key={entry.rank}
                      className={`flex flex-col sm:flex-row sm:items-center p-3 sm:p-4 mb-3 rounded-xl shadow-sm bg-white border overflow-x-hidden transition-all hover:shadow-lg ${entry.rank <= 3 ? 'border-blue-300' : 'border-gray-200'}`}
                    >
                      <div className="flex flex-wrap items-center w-full">
                        <span className={`w-7 h-7 flex items-center justify-center rounded-full font-bold text-sm mr-1 ${
                          entry.rank === 1 ? 'bg-yellow-100 text-yellow-700' : entry.rank === 2 ? 'bg-gray-200 text-gray-700' : entry.rank === 3 ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-600'
